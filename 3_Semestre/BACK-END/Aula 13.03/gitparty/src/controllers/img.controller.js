...
const prima = require("../data/prisma");
...
const cadastrar = async (req, res) => {
    try {
        ...
        const imgsFinal = `uploads/eventos/${idEvento}`;
        const caminhoFinal = `${imgsFinal}/${arquivo.filename}`;

        if (!fs.existsSync(pastaFinal)) {
            fs.mkdirSync(imgsFinal, {recursive: true});
        }
